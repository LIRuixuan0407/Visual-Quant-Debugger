...
export async function createPortfolioResearch(input: { name: string; factors: Array<{ research_id: string; weight: number; direction_override: 'HIGH' | 'LOW' | null }>; combination: CombinationMethod; filters: { minimum_liquidity: number | null; maximum_volatility: number | null; require_factor_availability: boolean; include_symbols: string[]; exclude_symbols: string[] }; construction: { selection: 'TOP_N' | 'TOP_PERCENT'; top_n: number; top_percent: number; weighting: 'EQUAL_WEIGHT' | 'SCORE_WEIGHTED'; max_single_position_weight: number }; rebalance: RebalanceRule; gross_notional: number; initial_cash: number; fee_bps: number; slippage_bps: number }): Promise<PortfolioResearchRecord> { const created = await post<PortfolioResearchRecord>('/api/portfolio-research', input); await addCreatedObjectToCurrentWorkspace('PORTFOLIO_RESEARCH', created.portfolio_research_id); return created }
