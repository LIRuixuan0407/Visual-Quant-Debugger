...
test('keeps parameter, stress, statistical, regime, fingerprint, and What-if diagnostics', async () => {
  ...
  expect(fingerprintHeading.compareDocumentPosition(trainTestHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  ...
  expect(screen.getByText('View sensitivity table')).toBeInTheDocument()
  ...
  expect(within(costStressSection).getByText('View stress table')).toBeInTheDocument()
  expect(within(costStressSection).getByText('Highest friction').closest('div')).toHaveTextContent('20 bps')
  expect(within(delaySection).getByText('View execution-delay table')).toBeInTheDocument()
  expect(within(delaySection).getByText('Longest delay').closest('div')).toHaveTextContent('+2 bars')
  ...
  expect(screen.getByText('Best regime').closest('div')).toHaveTextContent('LOW / UPTREND')
  expect(screen.getByText('Weakest regime').closest('div')).toHaveTextContent('HIGH / DOWNTREND')
  fireEvent.click(screen.getByText('View regime table'))
...
test('renders unreliable dataset frequency as unsupported without an annualized chart', async () => {
  ...
  const volatilitySection = screen.getByRole('heading', { name: 'Volatility diagnostics' }).closest('section')
  expect(volatilitySection).not.toBeNull()
  expect(within(volatilitySection!).getAllByText('Unavailable')).toHaveLength(2)
})
