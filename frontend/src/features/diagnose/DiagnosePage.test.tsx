...
test('renders trace-bound train/test, real rerun comparisons, and deterministic observations', async () => {
  ...
  const sensitivityChart = screen.getByRole('img', { name: /Train and test Sharpe/ })
  expect(sensitivityChart).toBeInTheDocument()
  expect(screen.getByText('Lookback 5 · current')).toBeInTheDocument()
  fireEvent.keyDown(sensitivityChart, { key: 'ArrowRight' })
  expect(screen.getByText('Lookback 8')).toBeInTheDocument()
