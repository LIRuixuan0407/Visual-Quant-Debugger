...
it('renders all backend relationship evidence and submits both thresholds', async () => {
  ...
  expect(screen.getByRole('heading', { name: 'Incremental Information' })).toBeInTheDocument()
  expect(screen.getByRole('heading', { name: 'Factor Cluster' })).toBeInTheDocument()
  expect(screen.getByRole('heading', { name: 'PCA Factor Structure' })).toBeInTheDocument()
