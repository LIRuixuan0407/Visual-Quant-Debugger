...
interface Props {
  ...
  onRunDataAudit?: (researchId: string) => void
}
...
function InferenceSummary({ label, inference }: { label: string; inference: HorizonEvaluation['ic_inference'] }) {
  const { tr } = useI18n()
  if (!inference) return <div><span>{tr(label)}</span><strong>—</strong><small>{tr('Insufficient observations for inference.')}</small></div>
  const ci = inference.ci_lower == null || inference.ci_upper == null ? '—' : `[${inference.ci_lower.toFixed(4)}, ${inference.ci_upper.toFixed(4)}]`
  const q = inference.q_value == null ? '—' : inference.q_value.toFixed(4)
  return <div><span>{tr(label)}</span><strong>{inference.statistically_significant ? tr('SIGNIFICANT') : tr('NOT SIGNIFICANT')}</strong><small>{tr(inference.method)} · t={inference.t_stat?.toFixed(2) ?? '—'} · p={inference.p_value?.toFixed(4) ?? '—'} · q={q} · 95% CI {ci}</small></div>
}
...
export default function FactorLabPage({ datasets, onOpenHistorical, onOpenReplay, onRunComplete, onRunDataAudit }: Props) {
  ...
  const [importOpen, setImportOpen] = useState(false); const [factorPath, setFactorPath] = useState(''); const [factorFile, setFactorFile] = useState<File | null>(null); const [factorClass, setFactorClass] = useState(''); const [importProof, setImportProof] = useState<{ checks: string[]; fingerprint: string } | null>(null)
  ...
  return <main className="discover-shell factor-lab-shell">
    ...
    {importOpen && <section className="local-import-panel" aria-label={tr('Import Factor')}><div className="import-copy"><span className="section-kicker">{tr('TRUSTED LOCAL PYTHON')}</span><h2>{tr('Register a VQDFactor source')}</h2><p>{tr('Upload a Python file for Docker-friendly import, or use a local path visible to the backend. The code runs with backend permissions; VQD does not provide a sandbox.')}</p></div><div className="import-fields"><label><span>{tr('Upload Python factor')}</span><input type="file" accept=".py,text/x-python" onChange={(event) => setFactorFile(event.target.files?.[0] ?? null)} /><small>{factorFile?.name ?? tr('Recommended for Docker and remote containers.')}</small></label><label><span>{tr('Factor source path')}</span><input value={factorPath} disabled={Boolean(factorFile)} onChange={(event) => setFactorPath(event.target.value)} placeholder="/home/me/factors/quality_momentum.py" /><small>{tr('Optional fallback for a path visible to the backend process.')}</small></label><label><span>{tr('Class name (optional)')}</span><input value={factorClass} onChange={(event) => setFactorClass(event.target.value)} placeholder="QualityMomentum" /></label><button className="primary-button" disabled={busy || (!factorFile && !factorPath.trim())} onClick={() => void submitFactorImport()}>{tr(busy ? 'Checking…' : 'Validate and import')}</button></div><div className="import-evidence"><span>{tr('Source file')}</span><i>→</i><span>{tr('SDK validation')}</span><i>→</i><span>{tr('Registry')}</span><i>→</i><span>{tr('Factor research')}</span></div></section>}
    ...
    <section className="factor-layout"><aside className="workspace-panel factor-ledger"><div className="section-heading"><h2>{tr('Research Ledger')}</h2><span>{ledger.length}</span></div><button className={!record ? 'selected new-research' : 'new-research'} onClick={() => setRecord(null)}>＋ {tr('New factor research')}</button>{ledger.map((item) => <button key={item.research_id} className={record?.research_id === item.research_id ? 'selected' : ''} onClick={() => { setBusy(true); void getFactorResearch(item.research_id).then((next) => { setRecord(next); setStage(next.revealed_stage) }).catch((reason) => setError(String(reason))).finally(() => setBusy(false)) }}><strong>{tr(item.name)}</strong><span>{tr(item.factor_id)} · {item.symbols} {tr('stocks')}</span><small>{dateOnly(item.created_at)} · {tr(item.revealed_stage)}</small></button>)}</aside>
      <div className="factor-main">{!record ? <section className="workspace-panel factor-builder"><div className="section-heading"><div><span className="section-kicker">{tr('DEFINE ONE HYPOTHESIS')}</span><h2>{tr('Start a factor study')}</h2></div><span className="bias-tag">{tr('POINT-IN-TIME')}</span></div>{eligible.length === 0 ? <div className="research-empty actionable"><strong>{tr('A multi-stock real dataset is required')}</strong><p>{tr('Download at least five real stocks in Historical Market before starting cross-sectional research.')}</p><button className="primary-button" onClick={onOpenHistorical}>{tr('Build real universe')}</button></div> : <>
        <div className="factor-definition-grid factor-lab-definition-grid">
          <label><span>{tr('Real-stock universe')}</span><select value={dataset?.dataset_id ?? ''} onChange={(event) => { const next = eligible.find((item) => item.dataset_id === event.target.value); setDatasetId(event.target.value); setDates(splitDates(next)) }}>{eligible.map((item) => <option value={item.dataset_id} key={item.dataset_id}>{tr(item.name)} · {item.symbols.length}</option>)}</select><small>{dataset?.symbols.join(' · ')}</small></label>
        ...
        <section className="workspace-panel factor-context"><div><span className="section-kicker">{tr('FACTOR RESEARCH')}</span><h2>{tr(record.name)}</h2><p>{record.universe.join(' · ')}</p></div><div className="factor-context-meta"><span className="status-badge">{tr(record.factor.origin ?? 'BUILT_IN')}</span><span className="status-badge">{tr(record.factor.category)}</span><span className="status-badge">{tr(record.factor.data_source)}</span><code>{record.research_id}</code><span>{record.factor_observation_count} {tr('factor observations')}</span>{onRunDataAudit && <button className="secondary-button" onClick={() => onRunDataAudit(record.research_id)}>{tr('Run Data Audit')}</button>}</div></section>
