...
import { cancelPaperOrder, createPaperAccount, createPaperSession, getMarketDataProviders, getPaperSession, getPaperTrace, isPaperSession, listPaperAccounts, listPaperSessions, transitionPaperSession } from '../../api/paper'
import { searchStocks } from '../../api/marketData'
import { useI18n } from '../../i18n/I18nProvider'
import type { MarketDataProviderStatus, PaperAccount, PaperExecutionMode, PaperSessionSnapshot, PaperTrace } from '../../types/paper'
...
function SessionHistory({ sessions, activeId, onOpen }: { sessions: PaperSessionSnapshot[]; activeId: string | null; onOpen: (id: string) => void }) {
  ...
  return <section className="workspace-panel live-history"><div className="section-heading"><h2>{tr('Recent Paper Sessions')}</h2><span>{sessions.length} {tr('retained locally')}</span></div>
    {sessions.length === 0 ? <p className="empty-state">{tr('No live paper sessions have been created.')}</p> : <div className="live-session-table" role="table">
      <div className="live-session-row header" role="row"><span>{tr('Start')}</span><span>{tr('Strategy')}</span><span>{tr('Symbols')}</span><span>{tr('Provider')}</span><span>{tr('Status')}</span><span>{tr('Equity')}</span></div>
      {sessions.map((session) => <button className={`live-session-row ${activeId === session.session_id ? 'selected' : ''}`} key={session.session_id} onClick={() => onOpen(session.session_id)}>
        <code>{formatTimestamp(session.started_at ?? session.created_at).date}</code><span>{tr(session.strategy_name)}</span><code>{session.symbols.join(' / ')}</code><code>{session.provider.toUpperCase()} · {session.feed.toUpperCase()}</code><span>{tr(session.status)}</span><strong>{formatCurrency(session.account.equity)}</strong>
      </button>)}
    </div>}
  </section>
}
