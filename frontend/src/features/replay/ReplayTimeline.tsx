...
function ReplayTimeline({ events, selectedEventId, onSelect }: ReplayTimelineProps) {
  const { tr } = useI18n()
  const chart = useMemo(() => {
    ...
    <section className="timeline-section" aria-labelledby="timeline-title">
      ...
      >
        ...
        {chart.map(({ event, x, y }) => {
          ...
          const label = `Select ${formatTimestamp(event.timestamp).date}${isSignal ? ', signal' : ''}${isExecution ? ', execution' : ''}`
          return (
