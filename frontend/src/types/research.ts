...
export interface CostPreview { gross_return: number; fees: number; slippage: number; spread_cost?: number; market_impact?: number; net_return: number; turnover: number; max_drawdown: number; positions: number; rebalance_count: number }
...
export interface PortfolioResearchRecord {
  portfolio_research_id: string; name: string; created_at: string; dataset_id: string; dataset_fingerprint: string; universe: string[]; factor_refs: PortfolioFactorRef[]; factor_ids: string[]; factor_names: string[]; combination: CombinationMethod; filters: { minimum_liquidity: number | null; maximum_volatility: number | null; require_factor_availability: boolean; include_symbols: string[]; exclude_symbols: string[] }; construction: { mode?: 'LONG_ONLY' | 'LONG_SHORT'; selection: 'TOP_N' | 'TOP_PERCENT'; top_n: number; top_percent: number; weighting: 'EQUAL_WEIGHT' | 'SCORE_WEIGHTED'; max_single_position_weight: number }; rebalance: RebalanceRule; gross_notional: number; initial_cash: number; fee_bps: number; slippage_bps: number; spread_bps?: number; market_impact_bps?: number; revealed_stage: ResearchStage; stages: PortfolioStageResult[]; strategy: { strategy_id: string; source_fingerprint: string } | null
}
